...
class FoodifySupabaseUser extends BaseAuthUser {
  FoodifySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> foodifySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FoodifySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
